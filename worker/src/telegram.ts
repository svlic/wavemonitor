import type { AlertDecision, EnabledSource, TelegramCredentials } from "./types";
...
export async function sendTelegram(credentials: TelegramCredentials | null, text: string): Promise<{ status: "sent" | "failed"; messageId: string | null; safeError: string | null } | null> {
  if (credentials === null) return null;
  let lastStatus = 502;
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      const response = await fetch(`https://api.telegram.org/bot${credentials.token}/sendMessage`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ chat_id: credentials.chatId, text, parse_mode: "MarkdownV2" }) });
