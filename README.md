...
# 可选：所有行情数据源使用的 SOCKS5 代理；推荐 socks5h，让 DNS 查询也经过代理
# WAVEMONITOR_SOCKS5_PROXY=socks5h://proxy-host:1080
...
| `WAVEMONITOR_SOCKS5_PROXY` | 从宿主机 `.env` 或环境传入 | 可选；代理 YFinance、Binance 和 Hyperliquid 的行情轮询及标的查询；推荐使用 `socks5h://` |
...
| `WAVEMONITOR_SOCKS5_PROXY` | 无 | 可选；所有行情数据源使用的 `socks5://` 或 `socks5h://` 代理 URL，推荐 `socks5h://` 以代理 DNS 查询 |
