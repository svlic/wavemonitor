...
| POST | `/api/instruments` | Creates an instrument and source mappings. `alert_mode` is `static` (default) or `fixed_drawdown`. In `static` mode, at least one of `support` / `resistance` is required and `high_water` / `fixed_drawdown` must be omitted. In `fixed_drawdown` mode, `high_water` and `fixed_drawdown` are required as absolute price amounts, `support` is derived as `high_water - fixed_drawdown` and must not be sent, and `resistance` is optional. `near_support_threshold` is required only when support is set (including derived support); `risk_reward_threshold` is required only when both levels are set. Inapplicable thresholds may be `null`. Returns **409** with `detail: "Source mapping already exists"` when the request contains duplicate source identities for the same instrument (same `provider`, `market_type`, and normalized `symbol`) or when the DB unique constraint is violated; the transaction rolls back (no partial instrument). |
