...
| PUT | `/api/instruments/{instrument_id}` | Replaces instrument fields and source mappings atomically. Uses the same conditional level/threshold contract and **409** semantics as create; failed updates do not partially apply field changes. A successful replacement starts a new rule-evaluation cycle only when rule fields change (`alert_mode`, `supports`, `resistances`, `high_water`, `fixed_drawdown`, `near_support_threshold`, `risk_reward_threshold`). Name, enabled, and source-mapping-only replacements keep the current cycle and `LastRuleState`. |
...
| GET | `/api/instruments/{instrument_id}/status` | Returns the instrument enabled flag, per-source in-memory latest price/error/`last_invalid_state` (rule invalid reason when price is not above support), and recent alerts for that instrument. Latest price/error state is empty after process restart until that source is polled. |
| GET | `/api/prices/latest` | Returns in-memory latest successful prices for **enabled** instruments and **enabled** source mappings only (paused instruments are omitted). Required boolean fields `support_breached` / `resistance_broken` are sticky per source only within the instrument's current rule-evaluation cycle: events from before the latest rule-field `PUT` are ignored, while a post-edit breach/breakout sets its flag for the rest of that cycle. Empty when none qualify or before the first successful poll after process start. |
| GET | `/api/alerts` | Returns recent alert events. Empty when no alerts exist. |
| GET | `/api/source-errors` | Returns collection-level source mappings whose most recent in-memory polling attempt is an error. Empty when no source errors exist; errors reset on process restart. |
| GET | `/api/runtime` | Returns scheduler/provider/Telegram readiness, polling counters, alert/delivery counters, and last tick timestamps. The backward-compatible `observations_written` field counts successful in-memory latest-price updates; it does not represent database writes. |
