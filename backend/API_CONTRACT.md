...
| POST | `/api/instruments` | Creates an instrument and source mappings. At least one of `support` / `resistance` is required. `near_support_threshold` is required only when `support` is set; `risk_reward_threshold` is required only when both levels are set. Inapplicable thresholds may be `null`. Returns **409** with `detail: "Source mapping already exists"` when the request contains duplicate source identities for the same instrument (same `provider`, `market_type`, and normalized `symbol`) or when the DB unique constraint is violated; the transaction rolls back (no partial instrument). |
| PUT | `/api/instruments/{instrument_id}` | Replaces instrument fields and source mappings atomically. Uses the same conditional level/threshold contract and **409** semantics as create; failed updates do not partially apply field changes. |
